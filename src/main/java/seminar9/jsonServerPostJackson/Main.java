package seminar9.jsonServerPostJackson;

/*
 Создать простое веб-приложение, которое предоставляет RESTful API для управления пользователями. Приложение использует com.sun.net.httpserver.HttpServer для создания HTTP-сервера и библиотеку Jackson для обработки данных в формате JSON.
     1. Запуск:
        Когда вы запускаете ваше приложение, оно создает HTTP-сервер, который слушает порт 8080 и ожидает запросов.
        ...
        DELETE запрос на /users/{id}: Удаляет пользователя по указанному ID и возвращает сообщение об успешном удалении в формате JSON.
